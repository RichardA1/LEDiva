...
LEDiva_NeoPixel::LEDiva_NeoPixel(uint16_t n, uint8_t p, neoPixelType t) :
...
LEDiva_NeoPixel::LEDiva_NeoPixel() :
...
void LEDiva_NeoPixel::begin(void) {
  if(pin >= 0) {
    pinMode(pin, OUTPUT);
...
void LEDiva_NeoPixel::updateLength(uint16_t n) {
...
void LEDiva_NeoPixel::updateType(neoPixelType t) {
...
void LEDiva_NeoPixel::show(void) {
...
void LEDiva_NeoPixel::setPin(uint8_t p) {
...
void LEDiva_NeoPixel::setPixelColor(
...
void LEDiva_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t LEDiva_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Convert separate R,G,B,W into packed 32-bit WRGB color.
// Packed format is always WRGB, regardless of LED strand color order.
uint32_t LEDiva_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  return ((uint32_t)w << 24) | ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t LEDiva_NeoPixel::getR(uint16_t n) const{
...
// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t LEDiva_NeoPixel::getG(uint16_t n) const{
  if(n >= numLEDs) return 0; // Out of bounds, return no color.
  uint8_t *p;
    p = &pixels[n * 3];
      return p[gOffset];
...
// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t LEDiva_NeoPixel::getB(uint16_t n) const{
  if(n >= numLEDs) return 0; // Out of bounds, return no color.
  uint8_t *p;
    p = &pixels[n * 3];
      return p[bOffset];
...
uint8_t *LEDiva_NeoPixel::getPixels(void) const {
  return pixels;
}

uint16_t LEDiva_NeoPixel::numPixels(void) const {
  return numLEDs;
}
...
void LEDiva_NeoPixel::setBrightness(uint8_t b) {
...
uint8_t LEDiva_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}


void LEDiva_NeoPixel::clear() {
  memset(pixels, 0, numBytes);
}
