...
#ifndef LEDIVA_NEOPIXEL_H
#define LEDIVA_NEOPIXEL_H
...
// the same item.  The third parameter to the LEDiva_NeoPixel
...
class LEDiva_NeoPixel {

 public:

  // Constructor: number of LEDs, pin number, LED type
  LEDiva_NeoPixel(uint16_t n, uint8_t p=6, neoPixelType t=NEO_GRB + NEO_KHZ800);
  LEDiva_NeoPixel(void);
  ~LEDiva_NeoPixel();
  ...
  uint32_t
    getR(uint16_t n) const;
  uint32_t
getG(uint16_t n) const;
  uint32_t
	getB(uint16_t n) const;
  inline bool
